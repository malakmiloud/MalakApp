...
namespace MalakApp
{
    public partial class VendorInsert : Form
    {
        ...
        private void submit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("نرجوا منك ملأ كل الحقول", "Captions", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (PhoneNO.Text.Length != 10)
            {
                MessageBox.Show("ادخل رقم هاتف صحيح", "Captions", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                ...
                if (i != 0)
                {
                    MessageBox.Show("تمت عملية اضافة مورد بنجاح", "Captions", MessageBoxButtons.OK, MessageBoxIcon.Information);

                }
                else
                {
                    MessageBox.Show("فشلت عملية اضافة المورد", "Captions", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                con.Close();
            ...
            {
                MessageBox.Show("لا يوجد مورد بهذا الاسم", "Captions", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                if(con != null)
